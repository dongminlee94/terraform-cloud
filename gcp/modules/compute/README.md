<!-- BEGIN_TF_DOCS -->


## Providers

| Name | Version |
|------|---------|
| <a name="provider_google"></a> [google](#provider\_google) | 4.75.1 |

## Resources

| Name | Type |
|------|------|
| [google_compute_instance.tf_gcp_prod_1](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_instance) | resource |
| [google_compute_instance.tf_gcp_prod_2](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_instance) | resource |
| [google_compute_instance.tf_gcp_prod_3](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/compute_instance) | resource |

## Inputs

| Name | Description | Type | Default | Required |
|------|-------------|------|---------|:--------:|
| <a name="input_nat_ip_1"></a> [nat\_ip\_1](#input\_nat\_ip\_1) | terraform google\_compute\_address resource 1 | <pre>object({<br>    address = string<br>  })</pre> | n/a | yes |
| <a name="input_nat_ip_2"></a> [nat\_ip\_2](#input\_nat\_ip\_2) | terraform google\_compute\_address resource 2 | <pre>object({<br>    address = string<br>  })</pre> | n/a | yes |
| <a name="input_nat_ip_3"></a> [nat\_ip\_3](#input\_nat\_ip\_3) | terraform google\_compute\_address resource 3 | <pre>object({<br>    address = string<br>  })</pre> | n/a | yes |
| <a name="input_service_account"></a> [service\_account](#input\_service\_account) | terraform google\_service\_account resource | <pre>object({<br>    email = string<br>  })</pre> | n/a | yes |
| <a name="input_ssh_pub_key"></a> [ssh\_pub\_key](#input\_ssh\_pub\_key) | ssh public key | `string` | `""` | no |
| <a name="input_subnetwork"></a> [subnetwork](#input\_subnetwork) | terraform google\_compute\_subnetwortk resource | <pre>object({<br>    id = string<br>  })</pre> | n/a | yes |
<!-- END_TF_DOCS -->
